...
method to display the 2 arrays in the test data. Use both the 'array' and the 'string' option. 
...
const pull = {
  ...
  registerNewAnswer() {
    let userAnswer = Number.parseInt(
      prompt(this.question + '\n' + this.option.join('\n'))
    );
    while (
      !userAnswer || userAnswer >= 0
        ? !(userAnswer < this.option.length)
        : false
    ) {
      userAnswer = Number.parseInt(
        prompt(
...
const newFunc1 = pull.displayResults.bind({ answers: [5, 2, 3] });
const newTest2 = pull.displayResults.bind({ answers: [1, 5, 3, 9, 6, 1] });
