...
format: https://geocode.xyz/52.508,13.381?geoit=json. Use the fetch API and promises to get the data. 
...
received about the provided location. Then, using this data, log a messuage like this to the 
...
fetch() does NOT reject the promise in this case. So create an error to reject the promise yourself, 
...
*/
/*
const errorHandler = (error, mess = `Something went wrong`) =>
  countriesContainer.insertAdjacentText('afterend', `${mess} ${error}`);
const fetchApis = function (url) {
  return fetch(url).then(response => {
    console.log(response);
    if (response.status === 403) throw new Error('Request Throttled');
    return response.json();
  });
};

const whereAmI = function (lat, lang) {
  fetchApis(`https://geocode.xyz/${lat},${lang}?geoit=json`)
    .then(data => {
      console.log(data);
      if (data.error && !data?.success)
        throw new Error(
          'Request Throttled. Over Rate limit: up to 0 per sec, wite a minute or so.'
        );
      console.log(`You are in ${data.city}, ${data.country}`);
      return fetchApis(`https://restcountries.eu/rest/v2/name/${data.country}`);
    })
    .then(data => {
      const [arrayData] = data;
      renderCountries(arrayData);
    })
