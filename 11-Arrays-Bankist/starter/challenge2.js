...
function calcAverageHumanAge(dogsAges) {
  const newDogsHumanLikeAge = dogsAges.map(dogAge => {
    if (dogAge <= 2) {
      return 2 * dogAge;
    } else {
      return 16 + dogAge * 4;
  ...
  const newDogsHumanLikeAge18 = newDogsHumanLikeAge.filter(
    filter18Age => filter18Age >= 18
  );
  const averageHumanAge = Math.trunc(
    newDogsHumanLikeAge18.reduce((acc, dogsAge) => acc + dogsAge, 0) /
      newDogsHumanLikeAge18.length
  );
  console.log(
    `All Dogs Ages: ${newDogsHumanLikeAge}\nDogs more than 18 Years Old Like Human Age: ${newDogsHumanLikeAge18}\nThe Average Age: ${averageHumanAge}`
  );
}
