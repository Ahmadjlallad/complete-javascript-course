...
and for each dog, calculate the recommended food portion and add it to the 
object as a new property. Do NOT create a new array, simply loop over the 
array.Formula: recommendedFood = weight ** 0.75 * 28. 
...
7. Create an array containing the dogs 
...
const littleOrTooMuch = (curFood, recommendedFood) => {
  if (curFood < recommendedFood) return 0.9;
  else if (curFood > recommendedFood) return 1.1;
};
// 1.
function doNotDistract() {
  dogs.forEach(
    dog =>
      (dog.recommendedFood = Number.parseFloat(
        (dog.weight ** 0.75 * 28).toFixed(2)
      ))
  );
  console.log(dogs);
  // 2.0
  dogs.forEach(dog => {
    if (dog.owners[0] === 'Sarah') {
      dog.curFood > dog.recommendedFood * 0.9 + dog.recommendedFood
        ? console.log(`${dog.owners[0]} dog is eating too little`)
        : null;
      dog.curFood < dog.recommendedFood * 1.1 + dog.recommendedFood
        ? console.log(`${dog.owners[0]} dog is eating too much`)
        : null;
    }
  });
  // 3.
  const [ownersEatTooMuch = [], ownersEatTooLittle = []] = dogs.reduce(
    (acc, dog) => {
      acc[
        littleOrTooMuch(dog.curFood, dog.recommendedFood) === 1.1 ? 0 : 1
      ].push(dog.owners);
      return acc;
    },
    [[], []]
  ...
  ownersEatTooMuch
    ...
    .forEach((_, i, arr) =>
      i === arr.length - 1
        ? console.log(
            `"${arr
              .slice(0, ownersEatTooMuch.flat().length)
              .join(' And ')} dogs eat too much!" and "${arr
              .slice(ownersEatTooMuch.flat().length)
  ...
  console.log(
    `"${ownersEatTooMuch
      .flat()
      .join(' And ')} dogs eat too much!" and "${ownersEatTooLittle
      .flat()
      .join(' And ')} dogs eat too little!"`
  );
}
...
function isEqual() {
  dogs.forEach(dog =>
    console.log(
      `Doge Owner ${dog.owners.join(' ')} RecommendedFood ${
...
function isOk() {
  dogs.forEach(dog =>
    console.log(
      `Doge Owner ${dog.owners.join(' ')} EXACTLY ${
...
// 8.
const sortedByRecommended = dogs.slice();
sortedByRecommended.sort((curr, last) => {
  console.log(curr.recommendedFood + ' - ' + last.recommendedFood);
  return curr.recommendedFood - last.recommendedFood;
});
